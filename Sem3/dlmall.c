...
//insert the block in the front of the list
...
//Adjust the size so it is even divided by 8 and have minimum size of 8
...
struct head *find(int size){
    if(flist == NULL){ //If we havnt created the freelist yet do so.
        flist = new();
        if(flist == NULL){
            return NULL;
        }
    ...
    while(block != NULL && (block->size < size)){ //searches for a size big enough to...
        block = block->next; //...accomodate the request
    }
    if(block == NULL){ //maybe we don't have a free block?
        return NULL;
    }
    detach(block); //detaches the block from the freelist
    if(block->size >= LIMIT(size)){ //if we could split the block
...
struct head *merge(struct head *block){
    ...
    if(block->bfree){ //is the block before free?
        struct head *befBlock = before(block); //get the block...
        detach(befBlock); //..and unlink it
        int newsize = befBlock->size + block->size + HEAD; //Calculate the total new merged size
        befBlock->size = newsize; // ...and set it to the block before
        aft->bsize = newsize; //...update the block after
        block = befBlock; 
    }
    if(aft->free){ // is the one after free as well?

        detach(aft); // unink it
        int aftsize = aft->size + block->size + HEAD; //calculate the size...
        block->size = aftsize; //..and update it
        struct head *nxtaft = after(block); //get the block after the fully merged updated block
        nxtaft->bsize = aftsize; //...and update that block
    }
    return block;
}
